package com.hypers.insight.hbaseendpoint.server;
...
/**
 * HBase RegionServer上Endpoint Coprocessor实现，主要实现对指定列的Count、MAX、MIN、SUM聚合操作
 */
public class MyFirstCoprocessorEndPoint extends MyFirstCoprocessor.AggregationService implements Coprocessor, CoprocessorService {
    ...
    public static int compare(Double l1, Double l2) {
        if (l1 == null ^ l2 == null) { //l1和l2异或 即一个为null 一个不为null
            return l1 == null ? -1 : 1; // either of one is null.
        } else if (l1 == null)
            return 0; // both are null
        return l1.compareTo(l2); // l1和l2都不为空，升序排列
    }
