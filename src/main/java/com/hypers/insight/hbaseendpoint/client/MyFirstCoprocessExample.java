package com.hypers.insight.hbaseendpoint.client;

import com.hypers.insight.hbaseendpoint.server.MyFirstCoprocessor;
import com.hypers.insight.hbaseendpoint.server.MyFirstCoprocessorEndPoint;
...
public class MyFirstCoprocessExample {
    ...
    public static void main(String[] args) {
        String table_name = "insight:bona_endpoint";
...
//            deleteCoprocessor(connection, table, MyFirstCoprocessorEndPoint.class);
            ...
            //为指定的表添加协处理器
//            String hdfspath = "hdfs://cdh-node1.cs1cloud.internal/bona/hbase/hbasedemo-1.0-SNAPSHOT.jar";
//            setupToExistTable(connection, table, hdfspath, MyFirstCoprocessorEndPoint.class);
//            System.out.println("Finish setup and start aggregation");

            //客户端调用Region端的协处理器 sales:MAX,sales:MIN,sales:AVG,sales:SUM,sales:COUNT
//            execFastEndpointCoprocessor(table, "info", "sales:MAX,sales:MIN,sales:AVG,sales:SUM,sales:COUNT");
//            execMaxEndpointCoprocessor(table, "info", "sales:MAX");
//            execMinEndpointCoprocessor(table, "info", "sales:MIN");
//            execSumEndpointCoprocessor(table, "info", "sales:SUM");
//            execCountEndpointCoprocessor(table, "info", "sales:COUNT");
            execAvgEndpointCoprocessor(table, "info", "sales:SUM,sales:COUNT, sales:AVG");
     ...
     * @return 返回表的总条数
     */
    public static long execCountEndpointCoprocessor(Table table, String family, String columns) {
        long start_t = System.currentTimeMillis();
        //定义总的 rowCount 变量
        AtomicLong totalRowCount = new AtomicLong();
        try {
     ...
     * @return 返回表的总条数
     */
    public static Double execMaxEndpointCoprocessor(Table table, String family, String columns) {
        long start_t = System.currentTimeMillis();
        //定义总的 rowCount 变量
        AtomicDouble maxValue = new AtomicDouble(Double.MIN_VALUE);
     ...
     * @return 返回表的总条数
     */
    public static Double execMinEndpointCoprocessor(Table table, String family, String columns) {
        long start_t = System.currentTimeMillis();
        AtomicDouble minValue = new AtomicDouble(Double.MAX_VALUE);
        try {
            Batch.Callback<MyFirstCoprocessor.MyCoprocessResponse> callback = new Batch.Callback<MyFirstCoprocessor.MyCoprocessResponse>() {
     ...
     * @return 返回表的总条数
     */
    public static Double execSumEndpointCoprocessor(Table table, String family, String columns) {
        long start_t = System.currentTimeMillis();
        //定义总的 rowCount 变量
        AtomicDouble sumValue = new AtomicDouble();
     ...
     * @return 返回表的总条数
     */
    public static Double execAvgEndpointCoprocessor(Table table, String family, String columns) {
        long start_t = System.currentTimeMillis();
        //定义总的 rowCount 变量
        AtomicLong totalRowCount = new AtomicLong();
        AtomicDouble sumValue = new AtomicDouble();
        ...
        System.out.println("totalRowCount：" + totalRowCount.longValue());
        System.out.println("sumValue：" + sumValue.doubleValue());
        Double avg = new DoubleColumnInterpreter().divideForAvg(sumValue.doubleValue(), totalRowCount.longValue());
